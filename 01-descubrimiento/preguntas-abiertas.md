...
| PA-028 | La contraparte no completa el proceso y el enlace de acceso expira. ¿El expediente se cierra, se emite un enlace nuevo automáticamente, o queda a la espera de que el responsable interno lo reactive? La §46 señala el caso pero no lo resuelve. | Cliente | `HU-010`, `HU-009` | Abierta | — |
| PA-029 | ¿Puede el Oficial de Cumplimiento decidir sobre un expediente que todavía tiene requisitos pendientes, dejándolo registrado como excepción, o el sistema debe impedirlo? | Cliente | `HU-015`, `HU-009` | Abierta — **alto impacto** | — |
| PA-030 | ¿Qué formatos de archivo y qué tamaño máximo por documento se aceptan? ¿Se admite que un mismo tipo documental llegue en varios archivos? | Cliente | `HU-013` | Abierta | — |
| PA-031 | ¿Quién entrega el enlace de acceso a la contraparte: lo envía la plataforma por correo, o lo copia el usuario operativo y lo hace llegar por su cuenta? Define si la Fase 1 necesita ya notificaciones automáticas. | Cliente | `HU-008`, `HU-010` | Abierta | — |
| PA-032 | ¿Por debajo de qué nivel de confianza una extracción con IA obliga a validación humana antes de poder usarse? ¿Lo fija cada organización cliente o hay un mínimo del producto? | Cliente | `HU-017`, `HU-020` | Abierta | — |
| PA-033 | ¿Qué umbral de similitud dispara una coincidencia en el matching de nombres, y quién lo configura? Un umbral bajo inunda de falsos positivos; uno alto deja pasar coincidencias reales. | Cliente | `HU-026` | Abierta — **alto impacto** | — |
| PA-034 | ¿La clasificación final de riesgo exige una segunda aprobación, y en qué casos? La §14 la menciona como posibilidad, no como regla. | Cliente | `HU-032` | Abierta | — |
| PA-035 | ¿Con qué periodicidad y ante qué disparadores se vuelve a consultar a las fuentes externas durante el monitoreo continuo? Es la principal fuente de costo variable del producto. | Cliente | `HU-040`, modelo de precios | Abierta — **alto impacto** | — |
| PA-036 | ¿Cuáles son los planes comerciales, sus precios y el cupo de consultas incluido en cada uno? | Cliente | `HU-046`, `ADR-0002` | Abierta — **bloqueante para la capa comercial** | — |
| PA-037 | Cuando una organización cliente agota su cupo, ¿se bloquea la consulta o se permite como excedente facturable? `ADR-0002` deja las dos puertas abiertas. | Cliente | `HU-047` | Abierta | — |
| PA-038 | ¿El alta de una organización cliente nueva es por autoservicio o siempre manual de nuestra parte? Define si la capa comercial necesita registro público. | Cliente + Camilo | `HU-046`, roadmap | Abierta | — |
| PA-039 | ¿Qué indicadores y qué tiempos de respuesta quiere ver el Oficial de Cumplimiento en su panel? La §22 da categorías, no métricas concretas. | Cliente | `HU-043` | Abierta | — |
